...
class TypesTableTableViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let type = dataModel.types[indexPath.row]
        cell.nombre.text = type.name
        cell.numero.text = String(type.races.count) + " razas"
        cell.icono.image = UIImage(named: type.icon)
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "Razas"{
            let rtvc = segue.destination as! RazasTableViewController
            if let ip = tableView.indexPathForSelectedRow{
               rtvc.type = dataModel.types[ip.row]
            }
